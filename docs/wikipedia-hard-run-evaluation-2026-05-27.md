...
- Hard failures are mostly route-quality failures, not click mechanics. The Antarctica run drifted through local fish taxonomy and geography; current prompts now label biological taxonomy links and warn against local classification loops unless the target asks for them.
- Long-article anchors remain weak. The agent reached `Snakehead (fish)#Culinary_use`, but prompt-visible content still looked like the top of the page, causing scroll/snapshot churn and a retreat to taxonomy.
...
- Done: prompt construction now includes lightweight route-helper hints over visible/current-page links, with regression coverage for Manga bridge links. A full multi-page graph search remains a possible future improvement.
...
- Same-page anchor clicks are now skipped when the URL already contains the target fragment; biological taxonomy loops now get prompt warnings, but repeated scroll churn still needs broader route-quality handling.
